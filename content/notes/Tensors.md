A tensor is defined as the linear map of the cross product of the cotangent space and the tangent space to the n-dimensional real space. A tensor takes r co-vectors and s vectors and gives us a number, $$T(\omega_{(1)},...,\omega_{(r)},V_{(1)},...,V_{(s)})$$denoted as a (r,s) tensor field, and these (r,s) tensors form a vector space.
A tensor is characterised by its components, $$T^{\mu_1...\mu_r}_{\nu_1...\nu_s}=T(dx^{\mu_1},...,dx^{\mu_r},\partial_{\nu_1},...,\partial_{\nu_s})$$
Under coordinate transformation, a tensor transforms in the same way as a vector in the upper indices and as a co-vector in the lower indices.
- Note that $$T^\mu_\nu = V^\mu\omega_\nu$$is an example of a Tensor Equation, an equation between the components of tensors of the same type, which is covariant under coordinate transformation.
