...
Since this is a conservative force, 
...
$$p^\mu=\int_V \rho^\mu$$
Thus, the relativistic generalisation of energy density is the #Stress-Energy tensor, a (2,0) tensor field which maps a covector to a vector as, 
...
$$\partial_t\rho+\partial_i\rho^i=0$$
which in integrated form translates to - the change in energy in a region is equal to the momentum flux through its boundaries. This is equivalent to saying that the change in #Stress-Energy tensor is zero. In a curved spacetime, 
$$\nabla_\mu T^{\mu\nu}=0$$
